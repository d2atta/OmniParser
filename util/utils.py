...
def get_caption_model_processor(
    ...
    if model_name == "blip2":
        ...
        # )
        # updated the model to not use flash-attn
        processor = AutoProcessor.from_pretrained(
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(suffix=".jpg", delete=False) as tmp_file:
                ...
                if result and len(result) > 0:
                    ...
                    print(page_result)
                    if (
                        "dt_polys" in page_result
                        and "rec_texts" in page_result
                        and "rec_scores" in page_result
                    ):
                        dt_polys = page_result["dt_polys"]
                        rec_texts = page_result["rec_texts"]
                        rec_scores = page_result["rec_scores"]
                        for i, (poly, rec_text, score) in enumerate(
            ...
            print(f"PaddleOCR 3.3.0处理出错，尝试旧版本API: {e}")
            try:
                result = paddle_ocr.ocr(image_np)
                if result and result[0]:
                    coord = [
                        item[0] for item in result[0] if item[1][1] > text_threshold
                    ]
                    text = [
                        item[1][0] for item in result[0] if item[1][1] > text_threshold
                    ]
                else:
                    coord = []
                    text = []
